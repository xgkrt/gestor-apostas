...
interface TipsterProfitData {
  tipster: string
  profit: number
}

interface ProfitByTipsterProps {
  data: TipsterProfitData[]
}
...
function CustomTooltip({
  ...
  return (
    <div className="rounded-xl border border-border bg-popover px-3 py-2 shadow-xl">
      <p className="text-xs font-semibold text-popover-foreground">{item.tipster}</p>
...
export function ProfitByTipster({ data }: ProfitByTipsterProps) {
  ...
  const chartData = useMemo<ChartDatum[]>(() => {
    return paginatedData.map((item) => {
      ...
      return {
        ...
        shortTipster: truncateLabel(item.tipster, 11),
        profit: item.profit,
        percentage,
      }
    })
  ...
  return (
    <Card className="bg-card p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-border shadow-sm flex flex-col h-full">
      <h4 className="text-base font-semibold text-foreground mb-4 sm:text-lg">Lucro por Tipster</h4>
        ...
        <div className="flex flex-1 flex-col min-h-0">
          <div className="h-[280px] w-full sm:h-[340px]">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <defs>
                  <linearGradient id="tipsterProfitPositiveGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#22c55e" />
                    <stop offset="100%" stopColor="#15803d" />
                  </linearGradient>
                  <linearGradient id="tipsterProfitNegativeGradient" x1="0" y1="0" x2="0" y2="1">
                ...
                <Bar dataKey="profit" barSize={34} radius={10}>
                  {chartData.map((entry) => (
                    <Cell
                      key={entry.tipster}
                      fill={entry.profit >= 0 ? "url(#tipsterProfitPositiveGradient)" : "url(#tipsterProfitNegativeGradient)"}
                    />
                  ))}
          ...
          <div className="mt-auto pt-4 border-t flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Mostrando {startIndex + 1} a {endIndex} de {data.length} {data.length === 1 ? "tipster" : "tipsters"}
            </div>
